	...
	try:
		...
		for i, dataset in enumerate(split_datasets):
			if hasattr(dataset, 'case_ids'):
				# Direct dataset with case_ids attribute
				splits.append(pd.Series(dataset.case_ids))
			elif hasattr(dataset, 'instances'):
				# Direct MultimodalDataset with instances
				case_ids = [inst['slide_id'] for inst in dataset.instances]
				...
				if hasattr(base_dataset, 'case_ids'):
					...
					if isinstance(base_case_ids, list):
						...
						case_ids = [base_case_ids[idx] for idx in indices]
					splits.append(pd.Series(case_ids))
				elif hasattr(base_dataset, 'instances'):
					# MultimodalDataset uses instances list with slide_id
					case_ids = [base_dataset.instances[idx]['slide_id'] for idx in indices]
					splits.append(pd.Series(case_ids))
				else:
...
class Trainer:
    ...
    def _evaluate_model(self, loader: DataLoader, model: nn.Module, drop_prob: Optional[float] = None) -> Tuple[Dict, float, float, Logger]:
        ...
        if hasattr(dataset_ref, 'case_ids'): # Direct dataset (has case_ids attribute)
            ...
            # Check if the inner dataset has case_ids or instances
            if hasattr(dataset_ref.dataset, 'case_ids'):
                case_ids_list = dataset_ref.dataset.case_ids
            elif hasattr(dataset_ref.dataset, 'instances'):
                # MultimodalDataset uses instances list with slide_id
                case_ids_list = [inst['slide_id'] for inst in dataset_ref.dataset.instances]
            else:
                raise ValueError(f"Expected dataset with case_ids or instances attribute, got {type(dataset_ref.dataset)}")
        elif hasattr(dataset_ref, 'instances'):
            # Direct MultimodalDataset without Subset wrapper
            case_ids_list = [inst['slide_id'] for inst in dataset_ref.instances]
        else:
            raise ValueError(f"Expected dataset with case_ids or instances attribute, got {type(dataset_ref)}")  
        patient_results = {}
