...
class CLAM(BaseModel):
    ...
    def _process_input_data(self, input_data: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        Args:
            input_data: Dictionary of channel tensors
            
        Returns:
            Concatenated tensor of all available channels
        """
        # Only use channels that exist in input_data (skip missing channels)
        available_tensors = [
            input_data[channel] 
            for channel in self.channels_used_in_model 
            if channel != 'wsi=reconstructed' and channel in input_data
        ]
        
        # Make sure we have at least one channel available
        if not available_tensors:
            raise ValueError(
                f"No valid channels found in input_data. "
                f"Expected: {self.channels_used_in_model}, "
                f"Got: {list(input_data.keys())}"
            )
        
        # Concatenate all available channel tensors
        h = torch.cat(available_tensors, dim=1).squeeze(0)
        return h
